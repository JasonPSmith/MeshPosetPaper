...
\begin{lem}
For any sets $A\subseteq B$ the interval $[s^A,s^B]$ is isomorphic to the boolean
 lattice~$B_{|B|-|A|}$. Therefore, $\mu(s^A,s^B)=(-1)^{|B|-|A|}$ and
$[s^A,s^B]$ is shellable.
\begin{proof}
We cannot remove any points from $s$, but we can
unshade any boxes from $B\setminus A$ in any order.
\end{proof}
\end{lem}
...
is obtained by taking a binary word $w$ put the positions that are $0$'s in increasing order
then the positions that are $1$ in increasing order, and shade everything southwest of $1$. 
So $f$ is a bijection.

The map $f$ is a bijection from $\mathcal{M}$ to $\mathcal{W}$ and it is straightforward
...
\begin{lem}
As $n$ tends to infinity the proportion of permutations of length $n$ that contain any of
...
can sum the over all the letters $i$.
...
the probability of containing any of the forbidden mesh pattern is bounded
...
of occurrences of $x$ in $y$, after adding the new points. This implies that if the
as the occurrences of $x$ in $y$ satisfy the conditions of \cref{lem:impureEdge}, the
