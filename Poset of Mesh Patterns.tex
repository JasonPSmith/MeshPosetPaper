...
shaded box then the resulting box will be unshaded.
...
\begin{lem}\label{lem:consecDel}
Consider a mesh pattern $m$, if deleting a point $i$ only removes shaded boxes by merging shaded boxes, then the set of points 
whose deletion gives the same mesh pattern as $i$ is a consecutive block where the each point when deleted also merges boxes in the same row, and these merged boxes form a consecutive block. 
\begin{proof}
Let $j$ be a position whose deletion gives the same mesh pattern as the deletion of $i$, and WLOG let $j<i$. Suppose there
is a position $z$ with $j<z<i$ and deleting $z$ gives a different mesh pattern.

Note that any pair of shaded boxes that deleting $m_i$ and $m_j$ must remove the same number of shaded boxes, either by merging
or deletion. I deleting $m_i$ merges boxes in row/column i, then deleting $m_j$ must also merge boxes in the same row/column and 
all boxes between these pairs of boxes must also be shaded, otherwise there is a non shaded boxes in a different position depending on
whether you merge boxes to left or right of it. This implies that deleting $m_z$ also merges boxes in the row/column. So any row/column
where boxes are merged is going to be exactly the same when deleting $m_z$ as when deleting $m_i$ or $m_j$.

Also note that when deleting $m_i$ and $m_j$ we can only remove shaded boxes by merging, unless $i=j-1$. 
This is because otherwise the shaded box removed by deleting $m_i$ is not present after deleting $m_i$ but is present after deleting
$m_j$ unless the box is removed by deleting both $m_i$ and $m_j$. Moreover, we cannot delete a shaded box when deleting $m_z$ otherwise this shaded box will be in a row one less when we delete $m_i$ instead of $m_j$, which contradicts their deletion giving the same mesh pattern. 

Therefore, deleting $m_z$ only removes shaded boxes by merging and it merges boxes from the same consecutive blocks as $m_i$ and $m_j$ which implies its deleton gives the same mesh pattern as the deletion of $m_i$ and $m_j$.
\end{proof}
\end{lem}
...
\begin{proof}
Suppose $x\lessdot y$ is an impure edge in $[m,p]$, and let $\eta$ be an occurrence of $x$ in $y$, with $a$ the letter $y$ not in $\eta$.
...
$t$ into the same place in~$\eta$, and~$\hat{x}$ is obtained by inserting a letter $s$ into $x$ whose
position and value is uniquely determined by~\hat{\eta}.
...
shaded boxes into more shaded boxes\footnote{A point inserted in box $(i,j)$ splits
every box in row $i$ and column $j$ into two boxes, except the box $(i,j)$ itself,
which splits into four boxes.}. And if 
a box is used by the occurrence, then the new occurrence uses all parts when it is split.

We show that every occurrence $\hat{\kappa}$ of $\hat{x}$ in $\hat{y}$
satisfies the conditions of \cref{lem:impureEdge}. This follows from \cref{lem:consecDel} because it is true for 
the $\hat{\eta}$ so must be true for all of $\hat{\kappa}$ as there are equivalent to deleting a letter that 
gives the same sub mesh pattern.
