...
\iffalse
Double check dimension, height, \dim and \h are correct throughout.

Comments to reviewer:
Changed dimension to height throughout.

Length of chain refers to number of edges and is defined as such when chain is defined in Section 2.1.

Clarified unbounded MF and changed reference to Sagan and Vatter.

The fact that disconnectivity inhibits shellability is well known but I am unable to find a source
where it is explicitly stated. However, I have added a reference to MS15 where it is mentioned
for the pure case.

Rewrote proof of Lemma 4.3
\fi
...
for all $1\le i \le k$. A poset is \emph{pure} (also known as \emph{ranked}) if all
...
\begin{lem}
For any mesh pattern $m$, we have $\h(1^\emptyset,m)=|\cl{m}|+|\sh{m}|$.
\begin{proof}
...
$\occX{a}{x}$ and $\occX{b}{x}$ in red. By \cref{lem:impureEdge} $\mX{a}{x}\lessdot a$ is impure,
...
where $\hat{y}$ is obtained by inserting the letter $t$ into $y$, $\hat{\eta}$ is obtained by inserting 
$t$ into the same place in $\eta$, and~$\hat{\eta}$ uniquely determines $\hat{x}$.
We claim that $\hat{x}\lessdot\hat{y}$ is an impure edge.

First note that inserting a letter into an occurrence that uses all the shaded boxes
creates a new occurrence with the same property.
Because inserting a point can only create more shaded boxes by splitting previously 
shaded boxes into two parts. And if 
a box is used by the occurrence, then the new occurrence uses both parts when it is split.

We show that every occurrence $\hat{\kappa}$ of $\hat{x}$ in $\hat{y}$
satisfies the conditions of \cref{lem:impureEdge}.
If $\hat{\kappa}$ contains $t$, then deleting $t$ gives an occurrence $\kappa$ of $x$ in~$y$,
which we know satisfies the conditions of \cref{lem:impureEdge}. 
So $\hat{\kappa}$ also satisfies the conditions, as after inserting $t$ into $\kappa$
the same letter is still being deleted, so merges shaded cells, and we know inserting a point
causes the new embedding to use all shaded boxes as well.

If $\hat{\kappa}$ does not contain $t$ 
then deleting $t$ from $\hat{y}$ gives
$\hat{x}$, which implies~$\hat{x}=y$.
It follows trivially that any occurrence of $\hat{x}$ in $y$
uses all the shaded boxes, and $\hat{\kappa}$ is obtained by inserting a point into
an embedding of $\hat{x}$ in $y$, thus $\hat{\kappa}$ uses all the shaded boxes. 
Suppose for a contradiction that deleting $t$ does not merge boxes.
This implies that the number of shaded boxes in $\hat{y}$ and~$\hat{x}$ is the same.
However, we know that the other occurrences of $\hat{x}$ in $\hat{y}$ do
merge a shaded box when deleting, which implies $\hat{y}$ has more shaded boxes than~$\hat{x}$.
This gives a contradiction. Therefore, $\hat{\kappa}$
satisfies the conditions to be impure.
...
%
%Suppose $x\lessdot y$ is an impure edge in $[m,p]$. Let $\eta$
%be an occurrence of~$y$ in $p$,
%where $j_1,j_2,\ldots,j_t$ are the positions of $y$ not used by $\eta$.
%Let $i$ be a position of $y$ whose deletion gives $x$.
%Let $b$ be the mesh pattern with $\cl{b}=\cl{p}$ whose shaded boxes are the those used by $\eta$.
%Also, let $a$ be the mesh pattern obtained by deleting $\eta_i$ from $b$, that is, the letter of $b$
%corresponding to $a_i$ according to $\eta$.
%
%Define $b^k$ as the mesh pattern obtained by deleting $b_{j_{k+1}},\ldots,b_{j_t}$ from~$b$,
%and obtain $a_k$ by deleting $\eta_i,b_{j_{k+1}},\ldots,b_{j_t}$ from $b$.
%So $b^0=y$, $a^0=x$, $b^t=b$ and~$a^t=a$.
%To get from $b^{k-1}$ to $b^k$ we insert a point $x_k$ into column $j_k$. 
%And we do a similar operation to get from $a^{k-1}$ to $a^k$.
%When we insert a point into a cell $c$, we split $c$ into four part
%and every cell in the same row or column as $c$ is split into two parts.
%
%By induction we show that every edge $a^k\lessdot b^k$ satisfies the conditions of \cref{lem:impureEdge}, so is
%an impure edge. We know that $a^0=x\lessdot y=b^0$ is impure so satisfies the conditions, and assume that
%$a^{k-1}\lessdot b^{k-1}$ satisfies the conditions.
%
%If $\phi$ contains $x_k$, then removing $x_k$ from $\phi$
%gives an occurrence $\hat{\phi}$ of $a^{k-1}$ in $b^{k-1}$. 
%Moreover, by the inductive argument $\hat{\phi}$ satisfies the conditions of 
%\cref{lem:impureEdge}, hence it straightforward to see that $\hat{\phi}$ also satisfies the conditions.
%
%If $\phi$ does not contain $x_k$, then deleting $x_k$ from $b^k$ gives
%$a^k$, which implies $a^k=b^{k-1}$. In which case
%it follows trivially that any occurrence of $a^k$ in $b^{k-1}$
%uses all the shaded boxes.
%When we insert $x_k$ into $a^k$ we cannot create new shaded boxes unless we
%split a pre-existing shaded box in two, and if a shaded box is being used before we split it
%then both the new boxes splitting it creates are also used. 
%This implies that every shaded box is used by $\phi$.
%
%Suppose for a contradiction that deleting $x_k$ does not merge boxes.
%This implies that the number of shaded boxes in $b^k$ and $b^{k-1}$ is the same.
%However, we know that the other occurrences of $a^k$ in $b^k$ do
%merge a shaded box when deleting, which implies $b^k$ has more shaded boxes than $b^{k-1}$.
%This gives a contradiction. Therefore, $\phi$ also
%satisfies the conditions to be impure.
%
%Thus all occurrences of $a^k$ in $b^k$ satisfy the conditions of 
%\cref{lem:impureEdge}, so~$a^k\lessdot b^k$ is impure.
%So by induction we get $a=a^t\lessdot b^t=b$ is impure.
\end{proof}
\end{lem}
...
contain an element of ${P_1\cup Q_1}$. Therefore, $P_1\cup Q_1$ and $P_2\cup Q_2$ are disconnected
