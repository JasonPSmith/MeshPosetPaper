...
a totally ordered subset $c_1<c_2<\cdots<c_{k+1}$, and the chain is \emph{maximal} if $c_i\lessdot c_{i+1}$,
...
Let $\pi$ be a permutation. For any sets $A\subseteq B$ the interval $[\pi^A,\pi^B]$ is isomorphic to the boolean
 lattice~$B_{|B|-|A|}$. Therefore, $\mu(\pi^A,\pi^B)=(-1)^{|B|-|A|}$ and
$[\pi^A,\pi^B]$ is shellable.
\begin{proof}
The elements of $[\pi^A,\pi^B]$ are exactly the mesh patterns $\pi^C$
...
\begin{lem}
The interval $[\pi^\emptyset,\sigma^\emptyset]$ is isomorphic to
$[\pi,\sigma]$ in $\mathcal{P}$, so $\muM(\pi^\emptyset,\sigma^\emptyset)=\muP(\pi,\sigma).$
\end{lem}
...
Therefore, we define the \emph{dimension} of a mesh pattern as $\dim(m)=|\cl{m}|+|\sh{m}|$ and we say an
