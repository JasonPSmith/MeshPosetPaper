...
Then deleting any position $k$ from $m$, with $i\le k\le j$, also gives $p$ and does not remove any shaded boxes.
...
First we consider any column~$c<i-1$. If deleting~$i$ or~$j$ removes a shaded boxes then this is true for both~$i$ and~$j$,
...
It remains to consider the rectangle given by the boxes~$(c,r)$ with~$i-1\le c\le j$ and~$\pi_i-1\le r\le\pi_j$,
 ...
 The north east most pair~$(x-1,y-1)$ and~$(x,y)$ of this diagonal are either in columns~$j-1$ and~$j$ or rows~$m_j-1$ and~$m_j$, and are shaded.
...
First note that inserting a point into a mesh pattern can only create more shaded boxes by splitting previously 
 ...
 which splits into four boxes.}. So inserting a letter into an occurrence that uses all the shaded boxes
...
If $\hat{\psi}$ contains $t$, we show that removing $t$ 
 ...
 If $\hat{\psi}$ does not contain~$t$, then $\hat{x}=y$. Again the first paragraph
...
So all occurrences of $\hat{x}$ in $\hat{y}$ satisfy
