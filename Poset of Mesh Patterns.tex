...
a characterisation of the non-pure (or non-ranked) intervals of the mesh pattern poset.
...
$\eta$ is called an \emph{occurrence} of $\sigma$ in $\pi$.
...
The \emph{M\"obius function} of an interval $[\alpha,\beta]$ of a poset is defined by:
...
\begin{proof}
The elements of $[s^A,s^B]$ are exactly the mesh patterns $s^C$
...
\end{cases}.$$
\begin{proof}
The first two cases are trivial. By \cref{lem:max} we know that
...
of such a sequence maps to an adjacency tail.  Combining this with the fact that if there are no adjacency tails, then the letters before the descent must be all the even letters of which there are $\lfloor\frac{n}{2}\rfloor$, completes the proof.
\end{proof}
\end{lem}
...
shaded box so $\dim(p)-\dim(m)\ge2$. So it suffices to show that there is no $z$ such
...
and $\sh{b}$ are the shaded boxes used by $\eta$. Let $a$ be the mesh pattern obtained from $b$
...
of occurrences of $x$ in $y$, after adding the new points. This implies that 
the occurrences of $x$ in $y$ satisfy the conditions of \cref{lem:impureEdge} if and only if the
occurrences of $a$ in $b$ satisfy the same conditions. So \cref{lem:impureEdge} implies $a\lessdot b$ is an
impure edge.
\end{proof}
\end{lem}
...
intervals of the permutation poset are not shellable in $\mathcal{P}$. We can generalise the direct sum operation to
...
sum of smaller permutations. We can generalise this to mesh patterns.
\begin{defn}
