...
First we consider a column~$c<i-1$. If deleting~$i$ or~$j$ merges shaded boxes then this is true for both~$i$ and~$j$,
 otherwise column $c$ in the resulting mesh patterns would have differing number of shaded boxes. If the deletion of $i$ or $j$ does not merge boxes,
...
If $\hat{\psi}$ contains $t$, we show that removing $t$ 
 ...
 If $\hat{\psi}$ does not contain $t$, then as $|\hat{x}|=|\hat{y}-1|$ we know that $t$ is the only letter not contained in $\hat{\psi}$,\todo{HU: This should be $|\hat{x}|=|\hat{y}|-1$?}
